...
exports.authController = {
    register: async (req, res) => {
        try {
            const { name, email, password, phoneNumber, role = "CLIENT", businessData, } = req.body;
    ...
    login: async (req, res) => {
        try {
            ...
            const expectedRole = userType.toUpperCase() === "BUSINESS" ? "BUSINESS" : "CLIENT";
    ...
    oauthLogin: async (req, res) => {
        try {
            const { email, name, provider, providerId, image, role = "CLIENT", } = req.body;
            if (!email || !name) {
                return res.status(400).json({
